...
} from "../interaction-conversion.js";
import type { Writable } from "../map-writable.js";
import { trace } from "../trace.js";
import type { InteractionMessage } from "./split-stream.js";

export interface InteractionListener {
  sendUpdate(ctx: unknown, update: CoreInteractionUpdate): Promise<void>;
  query(ctx: unknown, query: CoreInteractionQuery): Promise<CoreInteractionResponse>;
}
...
export class InteractionController {
  ...
  private async handleInteractionUpdate(ctx: unknown, update: InteractionUpdate): Promise<void> {
    const updateCase = update.message.case;
    const coreUpdate = convertProtoToInteractionUpdate(update);
    debug("interaction update converted", {
      case: updateCase,
      type: coreUpdate?.type ?? null,
      forwarded: coreUpdate !== null,
    });
    if (coreUpdate) {
  ...
  private handleInteractionQuery(ctx: unknown, queryProto: InteractionQuery): void {
