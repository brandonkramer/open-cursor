import type { LocalGitExecutor } from "./git-executor.js";
...
export async function getGitRepoPath(
  ...
  try {
    const result = await gitExecutor.exec(workspacePath, ["rev-parse", "--show-toplevel"]);
...
export async function getGitBranch(
  ...
  try {
    const result = await gitExecutor.exec(repoPath, ["rev-parse", "--abbrev-ref", "HEAD"]);
...
export async function getGitRemoteUrl(
  ...
  try {
    const result = await gitExecutor.exec(repoPath, ["config", "--get", "remote.origin.url"]);
