...
import type { Auth } from "@open-cursor/protocol";
import { backoff } from "@open-cursor/protocol";
...
class AuthManager {
  ...
  public async login({
    ...
    return this.pollAuthenticationStatus({
      uuid,
      verifier,
...
const decodeJwt = (token: string): JwtPayload | null => {
  try {
    ...
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- JSON.parse returns unknown, type validated structurally
