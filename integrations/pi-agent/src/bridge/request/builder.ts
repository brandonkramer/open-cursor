...
import { type BlobStore, getBlobId } from "@open-cursor/client";
import {
  AgentClientMessage,
...
import {
  ...
  McpTools,
} from "@open-cursor/protocol/__generated__/agent/v1/mcp_pb.js";
...
function formatContentBlocks(blocks: unknown[]): string {
  ...
  for (const raw of blocks) {
    ...
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- content blocks from pi are unknown objects
...
function formatToolResultStep(msg: ToolResultMessage, state: CursorStateStore | undefined): string {
  ...
  while (i < messages.length) {
    ...
    while (i < messages.length && messages[i]?.role !== "user") {
      ...
      if (stepMsg.role === "assistant") {
        const text = reconstructAssistantText(stepMsg, state);
        if (text) {
    ...
    const agentTurn = new AgentConversationTurnStructure({
      // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- blob IDs are Uint8Arrays branded by proto
...
function buildMcpToolDefinitions(tools: Tool[] | undefined): McpToolDefinition[] {
  ...
  return advertisedTools.map((tool) => {
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- tool.parameters matches JsonValue structurally
    const jsonSchema = tool.parameters as Record<string, unknown> | undefined;
    const schemaValue: JsonValue =
      jsonSchema && typeof jsonSchema === "object"
        ? // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- validated type above
...
export function buildRunRequest(params: BuildRunRequestParams): BuildRunRequestResult {
  const content =
    params.systemPromptOverride ?? params.context.systemPrompt ?? "You are a helpful assistant.";
  ...
  const turns = buildConversationTurns(params.context.messages, params.blobStore, params.state);
