...
import { type SessionMetadata, fromHex, toHex } from "@open-cursor/client";
...
export const loadBlobsFromDisk = async (
  ...
  try {
    const text = await fs.readFile(getBlobsFilePath(baseDir, sessionId), "utf-8");
    ...
    if (parsed?.version !== 1 || !Array.isArray(parsed.blobs)) {
      return new Map();
    }
    const map = new Map<string, Uint8Array>();
    for (const entry of parsed.blobs) {
      if (!entry || typeof entry.id !== "string" || typeof entry.data !== "string") continue;
      try {
...
export const loadMetaFromDisk = async (
  ...
  try {
    const text = await fs.readFile(getMetaFilePath(baseDir, sessionId), "utf-8");
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- JSON.parse returns unknown
    const parsed = JSON.parse(text) as MetaFile;
    if (parsed?.version !== 1 || typeof parsed.agentId !== "string") {
      return null;
    }
    return {
      ...
      // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- narrowing parsed.mode to known union
      mode: (parsed.mode as SessionMetadata["mode"]) ?? "default",
      ...(parsed.lastUsedModel !== null &&
        parsed.lastUsedModel !== undefined && {
...
export const saveMetaToDisk = async (
  ...
  const file: MetaFile = {
    ...
    ...(metadata.lastUsedModel !== null &&
      metadata.lastUsedModel !== undefined && {
